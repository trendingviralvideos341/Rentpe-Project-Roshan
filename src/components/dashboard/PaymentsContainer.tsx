                                ...
                                {filtered.map(t => {
                                    ...
                                    return (
                                        <tr key={t.id} className={`transition-colors hover:bg-slate-50/50 ${isPaid ? "bg-green-50/10" : "bg-red-50/10"}`}>
                                            ...
                                            <td className="p-4">
                                                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${isPaid ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                                            ...
                                            <td className="p-4">
                                                {!showNote[t.id] ? (
                                                    <Button size="sm" variant="outline" className="h-8 text-[10px] font-bold rounded-xl border-2" onClick={() => setShowNote(p => ({ ...p, [t.id]: true }))}>
                                                        {isPaid ? "REVERSE PAYMENT" : "MARK AS PAID"}
                                                    </Button>
                                                ) : (
                                                    <div className="space-y-1.5 min-w-[200px] animate-in slide-in-from-top-2 duration-200">
                                                        <Input
                                                            className="h-8 text-[10px] rounded-xl border-2"
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1 no-scrollbar">
                                                                    {allRecords.length > 0 ? allRecords.map((r: any) => (
                                                                        <div key={r.id} className={`text-[10px] p-2 rounded-xl border-2 transition-colors ${r.paid ? "bg-green-50/50 border-green-100 text-green-700" : "bg-red-50/50 border-red-100 text-red-700"}`}>
                                                                ...
                                                                <div className="pt-2 border-t border-slate-100">
                                                                    ...
                                                                    <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1 no-scrollbar">
                                                                        {paymentNotes.map((note: any, i: number) => (
                                                                            <div key={i} className={`text-[9px] p-2 rounded-xl border-2 ${note.action === 'PAYMENT_MARKED_PAID' ? "bg-blue-50/50 border-blue-100 text-blue-700" : "bg-orange-50/50 border-orange-100 text-orange-700"}`}>
