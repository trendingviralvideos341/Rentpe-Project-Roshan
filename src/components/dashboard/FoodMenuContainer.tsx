...
import { Plus, Trash2, Utensils, Save, Building2 } from "lucide-react";
...
export function FoodMenuContainer() {
    ...
    if (properties.length === 0) {
        return (
            <div className="p-12 text-center border-2 border-dashed rounded-[32px] bg-slate-50/50">
                ...
                <p className="text-muted-foreground mt-2">You must be assigned to at least one property to manage food menus.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {!selectedPropertyId ? (
                ...
                <div className="p-8 text-center text-muted-foreground animate-pulse">Loading menu data...</div>
            ) : (
                <div className="grid gap-6">
                    {menu.map((dayItem) => (
                        <Card key={dayItem.day} className="rounded-[24px] border-2 border-slate-100 shadow-sm overflow-hidden group hover:border-primary/20 transition-all bg-white">
                            <CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
                                <CardTitle className="text-lg font-bold flex items-center gap-2">
                                    <Utensils className="h-4 w-4 text-primary" />
                                    {dayItem.day}
                                </CardTitle>
                                <Button size="sm" onClick={() => handleSave(dayItem.day)} disabled={saving} className="rounded-xl">
                                ...
                                <div className="grid md:grid-cols-3 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className={dayItem.breakfast ? "text-primary font-bold border-primary/20 bg-primary/5 rounded-xl" : "text-slate-700 rounded-xl"}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className={dayItem.lunch ? "text-primary font-bold border-primary/20 bg-primary/5 rounded-xl" : "text-slate-700 rounded-xl"}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className={dayItem.dinner ? "text-primary font-bold border-primary/20 bg-primary/5 rounded-xl" : "text-slate-700 rounded-xl"}
                                        />
                                    </div>
                                </div>
                            </CardContent>
