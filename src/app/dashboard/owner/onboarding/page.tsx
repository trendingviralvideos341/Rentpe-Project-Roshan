...
export default function OnboardingPage() {
    ...
    const filtered = bookings.filter(b => {
        ...
        const matchesStatus = filter === "ALL" || 
            (filter === "PENDING_PAYMENT" && (b.status === "APPROVED_PAYMENT_PENDING" || b.status === "APPROVED")) ||
                    ...
                    <div className="flex gap-2">
                        {([
                            ...
                            <Button
                                ...
                                className={`rounded-xl px-5 font-bold uppercase text-[10px] tracking-widest h-9 ${
                                    filter === val ? "bg-indigo-600 shadow-lg shadow-indigo-100" : "border-slate-200 text-slate-500"
                                }`}
                            >
                                {label}
                            </Button>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex bg-slate-100/50 p-1 rounded-xl w-fit border border-slate-200">
                    {([
                        ...
                        >
                            {label} ({
                                val === "ALL" ? bookings.length : 
                                val === "PENDING_PAYMENT" ? awaitingPayment : 
                                paidCount
                            })
                        </button>
                    ))}
