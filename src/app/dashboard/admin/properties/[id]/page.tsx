    ...
    return (
        <div className="space-y-6 pb-40 p-4 md:p-8 bg-slate-50/30">
                ...
                <CardContent className="p-5">
                    ...
                    {p.status === "VERIFYING_DOCUMENTS" && (
                        <div className="mt-4 pt-4 border-t flex items-center justify-between gap-4">
                            ...
                            <span className="text-sm font-black text-indigo-900 shrink-0">{verifiedCount}/{mandatoryDocs.length} Verified</span>
                        </div>
                    )}
                </CardContent>
            </Card>
            ...
            <div className="flex gap-0 bg-slate-100/80 p-1 rounded-2xl border border-slate-200">
                <button
                    className={`flex-1 py-2.5 px-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === "overview" ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700"}`}
                    onClick={() => setActiveTab("overview")}
                >Property Overview</button>
                <button
                    className={`flex-1 py-2.5 px-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === "verification" ? "bg-white shadow-sm text-indigo-700" : "text-slate-500 hover:text-slate-700"}`}
                    onClick={() => setActiveTab("verification")}
                >
                    <ShieldCheck className="h-3.5 w-3.5" />
                    Verification Documents
                    {verifiedDocs.length > 0 && (
                        <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${activeTab === "verification" ? "bg-indigo-100 text-indigo-700" : "bg-slate-200 text-slate-600"}`}>
                            {verifiedDocs.length}
                        </span>
                    )}
                </button>
