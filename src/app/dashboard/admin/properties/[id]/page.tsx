...
export default function AdminPropertyDetailPage() {
    ...
    // Audit System State
    const [viewerDoc, setViewerDoc] = useState<{ url: string; label: string; type: string; docKey?: string; isPhoto?: boolean } | null>(null);
    ...
    const handleAuditVerify = async () => {
        ...
        try {
            const docKey = viewerDoc.docKey || viewerDoc.type;
            if (viewerDoc.isPhoto) {
    ...
    const handleAuditUnverify = async () => {
        if (!viewerDoc || !property) return;
        setAuditLoading(true);
        try {
            const docKey = viewerDoc.docKey || viewerDoc.type;
            ...
            toast.success(`${viewerDoc.label} unverified.`);
            setViewerDoc(null);
        } catch {
    ...
    const handleQuickVerify = async (docKey: string, label: string, isPhoto: boolean) => {
        if (!property) return;
        try {
            if (isPhoto) {
                await togglePropertyDocumentVerification(property.id, docKey, true);
    ...
    const handleQuickUnverify = async (docKey: string, label: string) => {
        if (!property) return;
        try {
            await togglePropertyDocumentVerification(property.id, docKey, false);
            setVerifiedDocs(prev => prev.filter(d => d !== docKey));
    ...
    return (
        <div className="space-y-6 pb-40 p-4 md:p-8 bg-slate-50/30">
            ...
            {/* Tab Navigation */}
            <div className="flex gap-0 bg-slate-100/80 p-1 rounded-2xl border border-slate-200">
                <button
                    className={`flex-1 py-2.5 px-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${
                        activeTab === "overview" ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700"
                    }`}
                ...
                <button
                    className={`flex-1 py-2.5 px-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                        activeTab === "verification" ? "bg-white shadow-sm text-indigo-700" : "text-slate-500 hover:text-slate-700"
                    }`}
                ...
                >
                    ...
                    {verifiedDocs.length > 0 && (
                        <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${
                            activeTab === "verification" ? "bg-indigo-100 text-indigo-700" : "bg-slate-200 text-slate-600"
                        }`}>{verifiedDocs.length}</span>
                    )}
                </button>
            </div>
            ...
            {activeTab === "verification" && (
                <div className="space-y-5">
                    ...
                    {([
                        { label: "Owner Aadhaar", photos: aadhaarPhotos, category: "AADHAAR", isLegal: true, required: 2, desc: "Government ID - Aadhaar Card" },
                        { label: "Owner PAN Card", photos: panPhotos, category: "PAN", isLegal: true, required: 2, desc: "PAN Card" },
                        { label: "PG / Property License", photos: licencePhotos, category: "PG_LICENCE", isLegal: true, required: 2, desc: "Business license" },
                        { label: "Identity Check (Selfie)", photos: livePhoto, category: "LIVE_PHOTO", isLegal: true, required: 1, desc: "Live selfie verification" },
                        { label: "Building Photos", photos: buildingPhotos, category: "buildingPhotos", isLegal: false, required: 4, desc: "4 exterior/property photos required" },
                        { label: "Common Area", photos: commonAreaPhotos, category: "commonAreaPhotos", isLegal: false, required: 4, desc: "Hallway, lobby, or shared spaces" },
                        { label: "Rooms & Bathrooms", photos: roomsAndBathroomPhotos, category: "roomsAndBathroomPhotos", isLegal: false, required: 4, desc: "Individual room photos" },
                        { label: "Amenities", photos: amenitiesPhotos, category: "amenitiesPhotos", isLegal: false, required: 4, desc: "Amenity and facility photos" },
                        { label: "Parking Area", photos: parkingPhotos, category: "parkingPhotos", isLegal: false, required: 2, desc: "Parking facility" },
                    ] as { label: string; photos: string[]; category: string; isLegal: boolean; required: number; desc: string }[])
                        .filter(s => s.photos.length > 0)
                        .map(section => {
                            const sectionVerifiedCount = section.isLegal
                                ? (verifiedDocs.includes(section.category) ? 1 : 0)
                                : section.photos.filter((_, i) => verifiedDocs.includes(`${section.category}-${i}`)).length;
                            return (
                                <Card key={section.category} className={`border-2 shadow-sm rounded-3xl overflow-hidden bg-white ${
                                    section.isLegal ? "border-indigo-100" : "border-slate-100"
                                }`}>
                                    <div className={`p-4 border-b flex items-center justify-between ${
                                        section.isLegal ? "bg-indigo-50/60" : "bg-slate-50/60"
                                    }`}>
                                        <div>
                                            <h3 className="font-black text-slate-800 flex items-center gap-2 text-sm">
                                                {section.isLegal ? <Shield className="h-4 w-4 text-indigo-500" /> : <ImageIcon className="h-4 w-4 text-slate-400" />}
                                                {section.label}
                                                {section.isLegal && <Badge className="bg-indigo-100 text-indigo-700 border-indigo-200 text-[8px] font-black uppercase">Legal</Badge>}
                                            </h3>
                                            <p className="text-[11px] text-slate-500 font-medium mt-0.5">{section.desc} · {section.photos.length}/{section.required} uploaded</p>
                                        </div>
                                        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black border ${
                                            sectionVerifiedCount === section.photos.length
                                                ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                                                : "bg-amber-50 text-amber-600 border-amber-200"
                                        }`}>
                                            {sectionVerifiedCount === section.photos.length
                                                ? <><CheckCircle className="h-3.5 w-3.5" /> All Verified</>                                                : <>{sectionVerifiedCount}/{section.isLegal ? 1 : section.photos.length} Verified</>}
                                        </div>
                                    </div>
                                    <CardContent className="p-4">
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                            {section.photos.map((url: string, i: number) => {
                                                const docKey = section.isLegal ? section.category : `${section.category}-${i}`;
                                                const isVerified = verifiedDocs.includes(docKey);
                                                const cardLabel = `${section.label}${section.isLegal ? "" : ` ${i + 1}`}`;
                                                return (
                                                    <div key={i} className="space-y-1.5">
                                                        {/* Image Card */}
                                                        <div
                                                            className={`relative group rounded-2xl overflow-hidden bg-slate-100 aspect-square cursor-pointer border-2 transition-all ${
                                                                isVerified ? "border-emerald-400" : "border-slate-200 hover:border-slate-300"
                                                            }`}
                                                            onClick={() => { setViewerDoc({ url, label: cardLabel, type: section.category, docKey, isPhoto: !section.isLegal }); setIsReuploadMode(false); setReuploadNote(""); }}
                                                        >
                                                            <img src={url} alt={cardLabel} className="w-full h-full object-cover" />
                                                            {isVerified && (
                                                        ...
                                                        <div className="flex gap-1">
                                                            {isVerified ? (
                                                                <button
                                                                    className="flex-1 h-7 rounded-xl bg-red-500 hover:bg-red-600 text-white text-[9px] font-black uppercase tracking-wide transition-colors"
                                                                    onClick={() => handleQuickUnverify(docKey, cardLabel)}
                                                                >UNVERIFY</button>
                                                            ) : (
                                                                <button
                                                                    className="flex-1 h-7 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white text-[9px] font-black uppercase tracking-wide transition-colors"
                                                                    onClick={() => handleQuickVerify(docKey, cardLabel, !section.isLegal)}
                                                                >VERIFY</button>
                                                            )}
                                                            <button
                                                                className="flex-1 h-7 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-[9px] font-black uppercase tracking-wide transition-colors"
                                                                onClick={() => { setViewerDoc({ url, label: cardLabel, type: section.category, docKey, isPhoto: !section.isLegal }); setIsReuploadMode(true); setReuploadNote(""); }}
                                                            >REUPLOAD</button>
                                                        </div>
                                                    </div>
                                                );
            ...
            {/* 1. DOCUMENT AUDIT DIALOG */}
            {viewerDoc && (() => {
                const dialogDocKey = viewerDoc.docKey || viewerDoc.type;
                const isDialogVerified = verifiedDocs.includes(dialogDocKey);
                return (
                    <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-0 md:p-8" onClick={() => { if (!auditLoading) { setViewerDoc(null); setIsReuploadMode(false); setReuploadNote(""); } }}>
                        <div className="bg-white w-full h-full md:h-auto md:max-w-4xl md:rounded-[32px] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="px-6 py-4 border-b flex items-center justify-between bg-white">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <Badge className={`mb-0.5 text-[9px] font-black uppercase tracking-widest ${
                                            isDialogVerified ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-indigo-100 text-indigo-700 border-indigo-200"
                                        }`}>{isDialogVerified ? "Verified" : "Pending Verification"}</Badge>
                            ...
                            <div className="flex-1 overflow-auto bg-slate-100 flex items-center justify-center p-4 md:p-8 min-h-[40vh]">
                                <img src={viewerDoc.url} alt={viewerDoc.label} className="max-w-full max-h-[65vh] object-contain rounded-2xl shadow-xl border-4 border-white" />
                            </div>

                            {/* Footer — REJECT | APPROVE | REUPLOAD ... | ✕ */}
                                ...
                                {isReuploadMode ? (
                                    <div className="p-5 space-y-3 animate-in slide-in-from-bottom-2">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                className="flex-1 h-11 rounded-2xl border-2 border-slate-200 text-slate-600 text-[11px] font-black uppercase tracking-widest hover:bg-slate-50"
                                                onClick={() => setIsReuploadMode(false)}
                                            >← Back</button>
                                            <button
                                                disabled={auditLoading || !reuploadNote.trim()}
                                                className="flex-[2] h-11 rounded-2xl bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white text-[11px] font-black uppercase tracking-widest transition-colors"
                                    ...
                                    <div className="flex items-stretch">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 py-5 px-4 flex flex-col items-center justify-center gap-1 bg-red-50 hover:bg-red-100 border-r border-slate-100 text-red-600 transition-colors group"
                                            onClick={handleAuditUnverify}
                                        >
                                        ...
                                        <button
                                            disabled={auditLoading}
                                            className={`flex-[2] py-5 px-4 flex flex-col items-center justify-center gap-1 border-r border-slate-100 transition-colors ${
                                                isDialogVerified
                                        ...
                                        >
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest">
                                                {auditLoading ? "Processing…" : isDialogVerified ? "✓ Verified" : "Approve"}
                                            </span>
                                        </button>
                                        ...
                                        <button
                                            disabled={auditLoading}
                                            className="flex-1 py-5 px-4 flex flex-col items-center justify-center gap-1 bg-orange-50 hover:bg-orange-100 border-r border-slate-100 text-orange-600 transition-colors"
                                        ...
                                        {/* CLOSE — black */}
                                        <button
                                            className="px-6 py-5 flex flex-col items-center justify-center gap-1 bg-slate-900 hover:bg-black text-white transition-colors"
                                            onClick={() => { setViewerDoc(null); setIsReuploadMode(false); setReuploadNote(""); }}
                                        >
                                            <X className="h-5 w-5" />
