...
import {
    ...
    BedDouble, ShieldCheck, Shuffle, AlertTriangle
} from "lucide-react";
import { 
    getAdminBookings, 
    approveBooking, 
...
export default function AdminBookingsPage() {
    ...
    const openSharingModal = (booking: any) => {
    ...
    const handleSaveSharingType = async () => {
        if (!sharingModal || !newSharingType || !sharingRoomId) {
            toast.error("Select both sharing type and room.");
            return;
        }
                                                            ...
                                                            <td className="p-4">
                                                                <div className="flex justify-end gap-1">
                                                                    {(() => {
                                                                        ...
                                                                        if (hasRoom && ['APPROVED', 'ROOM_RESERVED', 'AGREEMENT_PENDING'].includes(s)) return (
                                                                            <>
                                                                                <Button size="sm" className="h-7 text-[10px] bg-purple-600 hover:bg-purple-700 font-bold flex items-center gap-1" onClick={() => openSharingModal(booking)}>
                        ...
                        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setSharingModal(null)}>
                            <div className="bg-white rounded-2xl p-6 w-full max-w-lg space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                                <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={() => setSharingModal(null)} className="h-8 w-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 font-bold">✕</button>
                                </div>

                                <div className="bg-amber-50 border border-amber-300 rounded-xl p-3">
                                    <p className="text-xs font-bold text-amber-800">⚠️ Current: <span className="font-black">{sharingModal.occupancy}</span> • {sharingModal.guestName}</p>
                                    <p className="text-[10px] text-amber-700 mt-0.5">Admin Override: Updating will notify the student automatically.</p>
                                </div>
                                ...
                                {newSharingType && (
                                    <div>
                                        ...
                                        >
                                            ...
                                            {rooms.filter(r => r.propertyId === sharingModal.propertyId && r.type === newSharingType).map(r => (
                                ...
                                <div className="flex gap-3">
                                    ...
                                    >
                                        {savingSharing ? "Saving..." : "✅ Save Change"}
                                    </Button>
